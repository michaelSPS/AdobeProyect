...
public class RegressionSteps {
    ...
    @And("^(?:I|The Client) proceeds? to checkout$")
    public void proceedToCheckOut() throws IOException {
        CartPage cartPage = new CartPage(driver);
        cartPage.clickProceedToCheckout("proceedToCheckoutButton");
    }

    @Then("^(?:I|The Client) should see the login page to continue$")
    public void shouldSeeLoginPage() throws IOException {
        CartPage cartPage = new CartPage(driver);
        Assert.assertTrue("No se visualiza la página de login.", cartPage.isLoginPageDisplayed());
    }
